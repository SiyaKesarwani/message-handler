...
class ConcreteAgent(Agent):
    ...
    def __init__(self, inbox, outbox, name):
        ...
        super().__init__(inbox, outbox, name) # calling the constructor of base class
    ...
    def handle_hello(self, message_content, name):
        ''' "hello" keyword handler '''
        print(f"Inbox of {name} (hello) filtered from other's Outbox: {message_content['content']}")

    def handle_crypto(self, message_content, name):
        ''' "crypto" keyword & transfer ERC20 handler'''
        print(f"Inbox of {name} (crypto) filtered from other's Outbox")
        self.transfer_token()
    ...
    async def generate_random_message(self):
        ...
        while True:
            ...
            print(f"{self.name} Generated message is ... {message_content}")
            type = "message"
    ...
    async def check_balance(self):
        ...
        while True:
            ...
            print(f"{self.name} ERC-20 Token Balance: {self.w3.from_wei(balance, 'ether')} tokens")
    ...
    def transfer_token(self):
        ...
        if balance >= one_unit:
            ...
            print(f"Source has enough tokens... Transferring tokens through {self.name}")
            print(f"{self.name} Token transfer sent! Transaction hash: {self.w3.to_hex(tx_hash)}")
        else:
            print(f"{self.name} Not enough tokens to transfer.")
